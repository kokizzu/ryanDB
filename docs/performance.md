...
Many of these changes add complexity or weaken durability guarantees. They are listed here as a roadmap, not as a mandate to turn this into a production database.

Status legend used throughout this document:

- [done] implemented; see [optimizations.md](performance/optimizations.md) for measured results.
...
These keep memory and disk bounded for long-lived deployments. On a 40,000-write / 500-key workload the on-disk log shrank ~30× (5.5 MB → 187 KB) and restart recovery roughly halved; see [optimizations.md](performance/optimizations.md). They improve steady-state and recovery rather than peak ops/sec on a fresh cluster.
